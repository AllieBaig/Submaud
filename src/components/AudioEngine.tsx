...
export default function AudioEngine() {
  ...
  const runHealthCheck = async () => {
    ...
    if (timeAutomation?.isEnabled && isPlayingRef.current && mainAudio) {
      ...
      if (step) {
        ...
        if (step.type === 'mainVolume' || step.type === 'volume') {
          updateMainAudioSettings({ volume: step.value });
  ...
  useEffect(() => {
    ...
    if (mainAudioRef.current) {
      ...
      const volume = settings.mainAudio.volume * layerGain * masterGainMultiplier * safetyMultiplier * masterAudioMultiplier * safeListeningCap * prioMain;
      mainAudioRef.current.volume = Math.min(1.0, Math.max(0.0, volume));
      
      // Sync Web Audio side
      if (mainGainRef.current && audioCtxRef.current) {
         mainGainRef.current.gain.setTargetAtTime(settings.mainAudio.volume * masterAudioMultiplier, audioCtxRef.current.currentTime, 0.1);
      }
    }
  }, [settings.audioTools.gainDb, settings.mainAudio.volume, settings.mainAudio.gainDb, settings.globalModes, settings.subliminal, settings.advancedAudio.calmControl.layerPriority, safetyMultiplier, settings.syncLab, isPlaying, settings.hzAutoScaling]);
