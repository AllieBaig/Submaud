...
const LayerInput = ({ label, value, onUpdate }: { label: string, value: number, onUpdate: (v: number) => void }) => (
  <div className="flex items-center justify-between py-2 group/row">
    <span className="text-[10px] font-bold text-system-secondary-label uppercase tracking-wider group-hover/row:text-system-purple transition-colors uppercase">{label}</span>
      ...
      <AnimatePresence>
        {showToast && (
          <motion.div
            ...
            className="absolute -top-6 left-1/2 -translate-x-1/2 z-[110] px-4 py-1.5 bg-system-green text-white text-[10px] font-black uppercase tracking-widest rounded-full shadow-lg flex items-center gap-2 paper-emboss whitespace-nowrap pointer-events-none"
          >
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 bg-gradient-to-r from-system-purple to-system-blue hover:opacity-90 text-white rounded-full text-xs font-black uppercase tracking-widest transition-all shadow-sm active:scale-95 paper-emboss"
          >
            ...
            >
              ...
              <div className="p-6 bg-secondary-system-background border-t border-separator">
                <button
                  ...
                  className="w-full py-4 bg-system-purple hover:opacity-90 disabled:opacity-50 text-white rounded-2xl text-xs font-black uppercase tracking-widest transition-all shadow-lg active:scale-95 paper-emboss"
                >
                  Save Profile
                </button>
              </div>
                    ...
                    <button
                      ...
                      className={`p-3 rounded-2xl transition-all shadow-md active:scale-95 paper-emboss flex items-center justify-center min-w-[40px] ${
                        isApplying === profile.id 
                          ...
                          : 'bg-system-purple hover:opacity-90 text-white'
                      }`}
                      title="Apply Profile"
                    >
                      {isApplying === profile.id ? (
