...
export const HzSelector = ({ value, onChange, color }: { value: number, onChange: (v: number) => void, color: string }) => {
  ...
  const colorClass = color === 'purple' ? 'text-system-purple' : 
                    color === 'blue' ? 'text-system-blue' : 
                    color === 'green' ? 'text-system-green' : 
                    color === 'amber' ? 'text-system-orange' : 
                    color === 'rose' ? 'text-system-red' : 
                    color === 'indigo' ? 'text-system-blue' : 
                    'text-system-orange';

  const bgActiveColorClass = color === 'purple' ? 'accent-system-purple' : 
                            color === 'blue' ? 'accent-system-blue' : 
                            color === 'green' ? 'accent-system-green' : 
                            color === 'amber' ? 'accent-system-orange' : 
                            color === 'rose' ? 'accent-system-red' : 
                            color === 'indigo' ? 'accent-system-blue' : 
      ...
      <div className="paper-card px-10 py-8 flex flex-col items-center gap-2 min-w-[220px] relative animate-in zoom-in-95 duration-300 paper-emboss">
        <div className="flex items-baseline gap-2">
          <NumericKeypadInput 
            ...
            style={{ color: value > 0 ? (color === 'purple' ? 'var(--system-purple)' : color === 'blue' ? 'var(--system-blue)' : color === 'green' ? 'var(--system-green)' : color === 'amber' ? 'var(--system-orange)' : color === 'rose' ? 'var(--system-red)' : 'var(--system-orange)') : 'currentColor' }}
          />
...
export const PhysicalSoundEngineUI = ({ phys, onChange, color }: { phys: any, onChange: (v: any) => void, color: string }) => {
  ...
  const activeBtnClass = color === 'red' ? 'bg-system-red text-white border-system-red' : 
                        color === 'amber' ? 'bg-system-orange text-white border-system-orange' : 
